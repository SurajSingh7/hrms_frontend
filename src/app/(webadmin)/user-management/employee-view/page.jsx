...
const EmployeeWizard = ({ onSuccess }) => {
  ...
  //   console.log("handleFileChange",info,type,);
  ...
  //   const maxFileSize = 3 * 1024 * 1024; // 3MB max file size
  
  //   // Ensure only one file is allowed
  //   if (info.fileList.length > 1) {
  //     message.error(`You can only upload one file for ${type}.`);
  
  //     // Reset the file list to the most recent file (first file only)
  //     const latestFile = [info.fileList[info.fileList.length - 1]];
  //     setFileList({ ...fileList, [type]: latestFile });
  //     return;
  //   }
  ...
  //   // Validate the file
  //   if (!file) {
  //     message.error(`No file selected for ${type}.`);
  //     return;
  //   }
  
  //   const isAllowedType = allowedTypes.includes(file.type);
  //   if (!isAllowedType) {
  //     message.error(`${file.name} is not a valid file type. Allowed types are JPEG, PNG, and PDF.`);
  //     setFileList({ ...fileList, [type]: [] }); // Clear file list on invalid type
  //     return;
  //   }
  
  //   const isAllowedSize = file.size <= maxFileSize;
  //   if (!isAllowedSize) {
  //     message.error(`${file.name} exceeds the maximum file size of 3MB.`);
  //     setFileList({ ...fileList, [type]: [] }); // Clear file list on invalid size
  //     return;
  //   }
  
  //   // Prepare the file for upload
  //   const formData = new FormData();
  //   formData.append(type, file.originFileObj || file);
  ...
  //     // Update the file list and additional file info on success
  //     setFileList({ ...fileList, [type]: [file] }); // Keep only one file in the file list
  //     setFilInfo(response.data.files[type]);
